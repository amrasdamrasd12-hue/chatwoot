...
<template>
  ...
  >
    <!-- Constrained wrapper: header + footer stay anchored, only the
         ...
         on small screens or when the message is very long. -->
    <div class="flex flex-col overflow-hidden max-h-[calc(85vh-4rem)]">
      ...
      <header class="flex shrink-0 items-center gap-3 pb-4">
        ...
        <h3
          id="spell-check-title"
          class="text-[17px] font-semibold tracking-tight text-n-slate-12"
        >
          {{ $t('CONVERSATION.REPLYBOX.SPELL_CHECK.TITLE') }}
        </h3>
      </header>

      <!-- Scrollable content area: corrected + original blocks -->
      ...
      <footer class="flex shrink-0 items-center justify-between gap-3 pt-4">
        <button
          type="button"
